...
#define put_be32(p, v) \
  do { *(unsigned int *)(p) = htonl(v); } while (0)
...
static inline uint32_t ror(uint32_t x, unsigned n) { return (x >> n) | (x << (32 - n)); }
...
static inline uint32_t gamma0(uint32_t x) { return ror(x, 7) ^ ror(x, 18) ^ (x >> 3); }

static inline uint32_t gamma1(uint32_t x) { return ror(x, 17) ^ ror(x, 19) ^ (x >> 10); }
...
static void blk_SHA256_Transform(blk_SHA256_CTX *ctx, const unsigned char *buf) {
  ...
  for (i = 0; i < 16; i++, buf += sizeof(uint32_t)) W[i] = get_be32(buf);
...
void blk_SHA256_Final(unsigned char *digest, blk_SHA256_CTX *ctx) {
  ...
  /* copy output */
  for (i = 0; i < 8; i++, digest += sizeof(uint32_t)) put_be32(digest, ctx->state[i]);
}