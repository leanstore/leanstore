...
auto main(int argc, char **argv) -> int {
  ...
  if (FLAGS_ycsb_payload_size == ycsb::BLOB_NORMAL_PAYLOAD) {
    ycsb = std::make_unique<YCSBKeyValue>(FLAGS_ycsb_record_count, FLAGS_ycsb_read_ratio, FLAGS_ycsb_zipf_theta, false,
                                          payloads, *db);
  } else {
    ycsb = std::make_unique<YCSBBlobState>(FLAGS_ycsb_record_count, FLAGS_ycsb_read_ratio, FLAGS_ycsb_zipf_theta, true,
                                           payloads, *db);
  }
