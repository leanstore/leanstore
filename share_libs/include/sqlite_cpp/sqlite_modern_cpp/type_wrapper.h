...
namespace sqlite {
	...
	inline std::monostate get_col_from_db(
		[[maybe_unused]] sqlite3_stmt* stmt, [[maybe_unused]] int inx, [[maybe_unused]] result_type<std::monostate>) {
			return std::monostate();
	}
	inline std::monostate get_val_from_db([[maybe_unused]] sqlite3_value *value, result_type<std::monostate>) {
		return std::monostate();
	}
#endif
