...
void DTTable::open()
{
   ...
   columns.emplace("dt_misses_counter", [&](Column& col) { col << sum_reset(WorkerCounters::worker_counters, &WorkerCounters::dt_misses_counter, dt_id); });
   ...
   columns.emplace("dt_skipped_leaf", [&](Column& col) { col << sum_reset(WorkerCounters::worker_counters, &WorkerCounters::dt_skipped_leaf, dt_id); });
   ...
   columns.emplace("dt_goto_page_shared", [&](Column& col) { col << sum_reset(WorkerCounters::worker_counters, &WorkerCounters::dt_goto_page_shared, dt_id); });
   ...
   for (u64 i = 1; i < WorkerCounters::VW_MAX_STEPS; i++) {
      columns.emplace("vw_version_step_" + std::to_string(i),
                      [&, i](Column& col) { col << sum_reset(WorkerCounters::worker_counters, &WorkerCounters::vw_version_step, dt_id, i); });
   }
   // -------------------------------------------------------------------------------------
   ...
   // -------------------------------------------------------------------------------------
   columns.emplace("cc_todo_chains", [&](Column& col) { col << sum_reset(WorkerCounters::worker_counters, &WorkerCounters::cc_todo_chains, dt_id); });
   columns.emplace("cc_todo_remove", [&](Column& col) { col << sum_reset(WorkerCounters::worker_counters, &WorkerCounters::cc_todo_remove, dt_id); });
   columns.emplace("cc_todo_updates", [&](Column& col) { col << sum_reset(WorkerCounters::worker_counters, &WorkerCounters::cc_todo_updates, dt_id); });
   columns.emplace("cc_todo_updates_versions_removed",
                   [&](Column& col) { col << sum_reset(WorkerCounters::worker_counters, &WorkerCounters::cc_todo_updates_versions_removed, dt_id); });
   // -------------------------------------------------------------------------------------
   columns.emplace("cc_rtodo_opt_staged",
                   [&](Column& col) { col << sum_reset(WorkerCounters::worker_counters, &WorkerCounters::cc_rtodo_opt_staged, dt_id); });
   columns.emplace("cc_rtodo_opt_executed",
                   [&](Column& col) { col << sum_reset(WorkerCounters::worker_counters, &WorkerCounters::cc_rtodo_opt_executed, dt_id); });
   columns.emplace("cc_rtodo_opt_considered",
                   [&](Column& col) { col << sum_reset(WorkerCounters::worker_counters, &WorkerCounters::cc_rtodo_opt_considered, dt_id); });
   columns.emplace("cc_rtodo_lng_executed",
                   [&](Column& col) { col << sum_reset(WorkerCounters::worker_counters, &WorkerCounters::cc_rtodo_lng_executed, dt_id); });
   columns.emplace("cc_rtodo_shrt_executed",
                   [&](Column& col) { col << sum_reset(WorkerCounters::worker_counters, &WorkerCounters::cc_rtodo_shrt_executed, dt_id); });
   columns.emplace("cc_rtodo_to_lng", [&](Column& col) { col << sum_reset(WorkerCounters::worker_counters, &WorkerCounters::cc_rtodo_to_lng, dt_id); });
   columns.emplace("cc_todo_1_break", [&](Column& col) { col << sum_reset(WorkerCounters::worker_counters, &WorkerCounters::cc_todo_1_break, dt_id); });
   columns.emplace("cc_todo_2_break", [&](Column& col) { col << sum_reset(WorkerCounters::worker_counters, &WorkerCounters::cc_todo_2_break, dt_id); });
   columns.emplace("cc_todo_wasted", [&](Column& col) { col << sum_reset(WorkerCounters::worker_counters, &WorkerCounters::cc_todo_wasted, dt_id); });
   // -------------------------------------------------------------------------------------
   ...
   columns.emplace("cc_fat_tuple_fallback",
                   [&](Column& col) { col << sum_reset(WorkerCounters::worker_counters, &WorkerCounters::cc_fat_tuple_fallback, dt_id); });
}
// -------------------------------------------------------------------------------------
void DTTable::next()
{
