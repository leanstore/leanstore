...
OP_RESULT scanAscVW(u8* start_key, u16 key_length, function<bool(u8* key, u16 key_length, u8* value, u16 value_length)> callback, function<void()> undo);
// starts at the key + 1
OP_RESULT scanDescVW(u8* start_key, u16 key_length, function<bool(u8* key, u16 key_length, u8* value, u16 value_length)> callback, function<void()> undo);
bool reconstructTupleVW(std::unique_ptr<u8[]>& start_payload, u16& payload_length, u8 worker_id, u64 lsn, u32 in_memory_offset);
