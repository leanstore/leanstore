...
bool BTreeNode::merge(leanstore::storage::BufferFrame::Header& ownHeader, u16 slotId, ExclusivePageGuard<BTreeNode>& parent, ExclusivePageGuard<BTreeNode>& right)
{
   ...
   if(!is_leaf){ // Inner Node
      spaceUpperBound += spaceNeeded(extraKeyLength, sizeof(SwipType), tmp.prefix_length);
   }
   ...
   right.bf()->header.tracker.merge(ownHeader.tracker);
...
// -------------------------------------------------------------------------------------
// This = right
void BTreeNode::split(leanstore::storage::BufferFrame::Header& ownHeader, ExclusivePageGuard<BTreeNode>& parent, ExclusivePageGuard<BTreeNode>& nodeLeft, u16 sepSlot, u8* sepKey, u16 sepLength)
{
   ...
   nodeLeft.bf()->header.tracker = ownHeader.tracker;
}
// -------------------------------------------------------------------------------------
