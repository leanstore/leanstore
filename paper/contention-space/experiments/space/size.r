...
")
res=sqldf("select * from res order by c_tag asc")
res$c_tag <- as.character(res$c_tag)
res$c_tag [ res$c_tag == "emails-rnd"] <- "Random - Emails"
res$c_tag [ res$c_tag == "emails-seq"] <- "Sorted - Emails"
res$c_tag [ res$c_tag == "urls-rnd"] <- "Sorted - URLs"
res$c_tag [ res$c_tag == "urls-seq"] <- "Random - URLs"
...
g <- ggplot(res, aes(label=size, x=reorder(type, -size), y=size)) +
    facet_wrap( ~ c_tag , scales="free_y") +
    ...
    labs (y = "Size [GiB]")
g
ggsave('../../tex/figures/size.pdf', width=3.3374, height = 2.5, units="in")
