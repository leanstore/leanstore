...
g <- ggplot(d, aes(t, tx, color=factor(symbol), group=factor(symbol))) +
    ...
    labs(x='Time [sec]', y = 'TPC-C throughput [K txns/s]') +
    scale_color_manual(guide=FALSE, breaks=c(1,3), values=c("black", "#619CFF"))+
    scale_shape_discrete(guide=FALSE)+
    theme_acm +
    theme(axis.title.y = element_text(hjust = 1.0)) +
    expand_limits(x=0,y=0) +
    annotate("text", x=1450, y=30000/1e3, label="Baseline", color ="black", size =7/.pt) +
    annotate("text", x=1450, y=55000/1e3, label="+XMerge", color = "#619CFF", size = 7/.pt)
g
...
dts=sqldf("select stats.c_su_merge, dts.* from dts, stats where stats.tpcc_warehouse_count=10000 and stats.c_pp_threads=4 and dts.c_hash=stats.c_hash and dts.t >= 100 and dts.t<500 and stats.t=1 ")

ggplot(d, aes(t, tx)) + facet_grid(col=vars(c_su_merge), row=vars()) + geom_line()

ggplot(dts, aes(t, dt_misses_counter)) + facet_grid(col=vars(c_su_merge), row=vars(dt_name)) + geom_line() + scale_y_log10()
