...
tx <- ggplot(d, aes(t, tx, color=factor(c_cm_split), group=factor(c_cm_split))) +
    ...
    theme_bw() +
    expand_limits(y=0) +
    facet_grid(row=vars(c_worker_threads), col=vars(c_zipf_factor, latest_window_ms))
#    facet_grid (row=vars(c_zipf_factor, latest_read_ratio, c_dram_gib), cols=vars(latest_window_gib,latest_window_offset_gib,latest_window_ms,  c_target_gib)) + xlim(0,30)
print(tx)
