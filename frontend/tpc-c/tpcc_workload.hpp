...
void loadCustomer(Integer w_id, Integer d_id)
{
  ...
  for (Integer i = 0; i < 3000; i++) {
    ...
    history.insert({WorkerCounters::myCounters().t_id, h_pk++, i + 1, d_id, w_id, d_id, w_id, now, 10.00, randomastring<24>(12, 24)});
  }
}
...
void paymentById(Integer w_id, Integer d_id, Integer c_w_id, Integer c_d_id, Integer c_id, Timestamp h_date, Numeric h_amount, Timestamp datetime)
{
  ...
  history.insert({WorkerCounters::myCounters().t_id, h_pk++, c_id, c_d_id, c_w_id, d_id, w_id, datetime, h_amount, h_new_data});
}
