...
function benchmark() {
    CSV_PATH="$(pwd)/spin.csv"
    rm -f $CSV_PATH
    (cd $EXEC_DIR; make -j latest)

    for FLAGS_worker_threads in 54 100 160; do
    for FLAGS_latest_read_ratio in 0; do
    for FLAGS_zipf_factor in 0.75; do
    for FLAGS_target_gib in 150; do # 25 50 75
    for FLAGS_latest_window_gib in 10; do
    for FLAGS_latest_window_offset_gib in 0.1; do
    for FLAGS_latest_window_ms in 100000; do
