...
template <>
struct fmt::formatter<leanstore::storage::LargePageTier> {
  template <typename ParseContext>
  constexpr auto parse(ParseContext &ctx) {
    return ctx.begin();
  }
  ...
  template <typename Context>
  auto format(leanstore::storage::LargePageTier const &page, Context &ctx) const {
    return fmt::format_to(ctx.out(), "[{0}:{1}]", page.start_pid, page.tier_index);
  }
};
