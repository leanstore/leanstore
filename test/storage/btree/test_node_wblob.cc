...
TEST_P(TestBTreeNodeWithBlob, NormalOperation) {
  ...
  for (size_t idx = 0; idx < BTreeNode::HINT_COUNT; idx++) {
    EXPECT_EQ(std::memcmp(random_blob[0].data(), node->hints[idx].data(), 4), 0);
  }
...
TEST_P(TestBTreeNodeWithBlob, TestSplitAndMerge) {
  ...
  EXPECT_EQ(new_leaf->space_used,
            blob_hdl_sz * (no_records - node->count) + blob_hdl_sz);  // all keys + left fence
  ...
  EXPECT_GE(new_leaf->FreeSpaceAfterCompaction(), BTreeNodeHeader::SIZE_UNDER_FULL);
