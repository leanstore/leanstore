...
TEST_F(TestBTreeNode, NormalOperation) {
  ...
  for (size_t idx = 0; idx < storage::BTreeNode::HINT_COUNT; idx++) {
    LOG_DEBUG("Clone's Hint[%ld]: %s - Original Hint[%ld]: %s", idx, fmt::format("{}", clone->hints[idx]).c_str(), idx,
              fmt::format("{}", node->hints[idx]).c_str());
    EXPECT_EQ(std::memcmp(clone->hints[idx].data(), node->hints[idx].data(), 4), 0);
  }
...
TEST_F(TestBTreeNode, NormalOperationWithPrefix) {
  ...
  for (auto &pair : data) {
    ...
    auto key_head = storage::BTreeNode::GetHead(pair.first.data() + node->prefix_len, key.size() - node->prefix_len);
    leng_t lower  = 0;
    leng_t upper  = node->count;
    node->SearchHint(key_head, lower, upper);
    ...
    std::memcpy(queried_key.data() + node->prefix_len, node->GetKey(slot_id), key.size() - node->prefix_len);
...
TEST_F(TestBTreeNode, TestSplit) {
  ...
  EXPECT_LE(sep_info.len, node->prefix_len + node->slots[node->count - 1].key_length);

  u8 last_key[node->prefix_len + node->slots[node->count - 1].key_length];
  std::memcpy(&last_key, new_leaf->GetPrefix(), new_leaf->prefix_len);
  std::memcpy(&last_key + new_leaf->prefix_len, new_leaf->GetKey(0), new_leaf->slots[0].key_length);
...
TEST_F(TestBTreeNode, TestSplitInnerNodes) {
  ...
  EXPECT_LE(sep_info.len, left->prefix_len + left->slots[left->count - 1].key_length);

  u8 parent_key[parent->prefix_len + parent->slots[parent->count - 1].key_length];
  std::memcpy(&parent_key, parent->GetPrefix(), parent->prefix_len);
  std::memcpy(&parent_key + parent->prefix_len, parent->GetKey(0), parent->slots[0].key_length);
