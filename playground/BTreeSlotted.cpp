...
void printInfos(BTreeNode* root, uint64_t totalSize)
{
  ...
  cout << "nodes:" << cnt << " innerNodes:" << countInner(root) << " space:" << (cnt * BTreeNodeHeader::pageSize) / (float)totalSize
       << " height:" << height(root) << " rootCnt:" << root->count << " bytesFree:" << bytesFree(root) << endl;
}
// -------------------------------------------------------------------------------------
